        ...
        <ul>
          ...
          >
            ...
            <span class="pmain-title3 pmain-height" style="width: 160px;">
              ...
              <span v-if="orderItem.status === 1"><button class="action-btn" @click="confirmOrder(orderItem.id)">确认完成</button></span>
            </span>
          </li>
        </ul>
      </div>
...
export default {
  ...
  methods: {
    fetchOrderList() {
      this.$axios.$get('/admin/core/order/userList').then((response) => {
    ...
    confirmOrder(id) {
      this.$axios.$get('/admin/core/order/confirmOrder/' + id).then((response) => {
        location.reload();
      })
    },
  }
